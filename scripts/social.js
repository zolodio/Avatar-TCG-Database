...
(function () {
  ...
  function refreshProfileHeader(profile) {
    var sec=$('socialSection'); if (!sec) return;
    // Find the header wrapper (first child div with flex align-items:center)
    var hdr=sec.querySelector('[id="socialAvatarInitial"]');
    var wrap=hdr ? hdr.closest('div') : sec.querySelector('div[style*="align-items:center"]');
    if (!wrap) return;

    var displayName=profile.display_name||profile.username;
    var sub=profile.bio ? profile.bio.slice(0,70) : 'Community member';
    ...
    if (profile.preferred_traits && profile.preferred_traits.length) {
      ...
      traitsHtml='<div style="display:flex;gap:4px;margin-top:5px;flex-wrap:wrap;">' +
        profile.preferred_traits.slice(0,3).map(function(t){
          return tmap[t]
            ? '<img src="'+esc(tmap[t])+'" title="'+esc(t)+'" style="width:18px;height:18px;opacity:0.85;" loading="lazy">'
            : '<span style="font-size:0.65rem;color:var(--text-muted);">'+esc(t)+'</span>';
        }).join('')+'</div>';
    }
    var chamberHtml=profile.favorite_chamber
      ? '<div style="font-size:0.65rem;color:var(--air);margin-top:3px;"><i class="fas fa-window-maximize" style="font-size:0.58rem;transform:scale(.9,1.7);margin-right:4px;"></i>'+esc(profile.favorite_chamber)+'</div>'
      : '';

    wrap.innerHTML =
      avatarHtml(profile,56) +
      '<div style="flex:1;min-width:0;">' +
        '<div style="font-family:\'Cinzel\',serif;font-weight:700;font-size:0.9rem;display:flex;align-items:center;flex-wrap:wrap;gap:4px;line-height:1.3;">'+
          esc(displayName)+(isPro?proBadge():'')+'</div>' +
        (displayName!==profile.username ? '<div style="font-size:0.68rem;color:var(--text-muted);margin-top:1px;">@'+esc(profile.username)+'</div>' : '') +
        '<div style="font-size:0.72rem;color:var(--text-secondary);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:210px;" title="'+esc(sub)+'">'+esc(sub)+'</div>' +
        traitsHtml+chamberHtml +
      '</div>' +
      '<button id="editProfileBtn" type="button" ' +
        'style="flex-shrink:0;padding:7px 13px;border-radius:8px;border:1px solid var(--border);background:var(--bg-card);color:var(--text-secondary);font-size:0.7rem;font-family:\'Nunito Sans\',sans-serif;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all 0.2s;">' +
        '<i class="fas fa-pen"></i> Edit' +
      '</button>';

    var editBtn=$('editProfileBtn');
    if (editBtn) {
      editBtn.addEventListener('mouseenter', function(){ this.style.borderColor='var(--zen)'; this.style.color='var(--zen)'; });
      editBtn.addEventListener('mouseleave', function(){ this.style.borderColor='var(--border)'; this.style.color='var(--text-secondary)'; });
  ...
  async function saveProfileChanges() {
    ...
    closeProfileEditor();
    refreshProfileHeader(currentProfile);
